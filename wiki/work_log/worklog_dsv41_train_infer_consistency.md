...
> **2026-09-22 补充（输入打桩，独立文档见 [worklog_dsv41_module_input_probe.md](worklog_dsv41_module_input_probe.md)）**——§9 的"混合态 caveat"（§9.6）已被消除，并给出放大机制：
> 18. **把模块输入换成引擎侧逐位相同的张量后，两个栈的模块自身仍差 ~0.5%，且与专家数无关**（attn 0.0050–0.0057，MoE 0.0054–0.0055；第 0 层注意力的输入本来就是逐位一致 → 那 0.57% 是纯核差）。
> 19. **端到端噪声的主体是这个 0.5% 被 MoE 的离散路由翻转放大出来的**：第 0 层 MoE 输入差 0.60% → 输出差 6.4%（10.7×），其中 **97% 的平方误差落在 4.7% 的"top-6 集合变了"的 token 上**；全层/两模型都是 **92–98%**。
> 20. **全部模块输入钉死后**：σ 从 0.32/0.56（32 专家）与 1.08/0.71（384 专家）掉到 **0.030–0.034**，`model.norm` 一律 **0.0070** → 存在与专家数无关的"输入一致地板"。**专家数只加强放大器（同输入差下翻转率 16%→31%），不改变核差**——这就是"kl 0.11→0.38"的机制。
...
- ~~输入打桩（把模块输入钉成引擎值）~~ → **已于 2026-09-22 完成**，见 [worklog_dsv41_module_input_probe.md](worklog_dsv41_module_input_probe.md)。
